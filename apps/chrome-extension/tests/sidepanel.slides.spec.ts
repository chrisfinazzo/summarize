...
test("sidepanel uses slide summaries when the summary stream finishes before slide payloads", async ({
  browserName: _browserName,
}, testInfo) => {
  const harness = await launchExtension(getBrowserFromProject(testInfo.project.name));
  ...
  try {
    ...
    await page.route("http://127.0.0.1:8787/v1/summarize/slides-run/events", async (route) => {
      await route.fulfill({
        status: 200,
        headers: { "content-type": "text/event-stream" },
        body: [
          "event: chunk",
          `data: ${JSON.stringify({ text: slidesMarkdown })}`,
          "",
          "event: done",
          "data: {}",
          "",
    ...
    await page.route(
      "http://127.0.0.1:8787/v1/summarize/slides-run/slides/events",
      async (route) => {
        ...
        await route.fulfill({
          ...
          body: [
            ...
            `data: ${JSON.stringify(
              buildSlidesPayload({
                sourceUrl: "https://www.youtube.com/watch?v=havana123",
                sourceId: "youtube-havana123",
                count: 2,
    ...
    await page.route("http://127.0.0.1:8787/v1/summarize/slides-run/slides", async (route) => {
      await route.fulfill({
        status: 200,
    ...
    await sendBgMessage(harness, {
      ...
      state: buildUiState({
        tab: {
          id: 1,
          url: "https://www.youtube.com/watch?v=havana123",
    ...
    await expect
      ...
      .toContain("portable classified device");
    await expect
      .poll(async () => (await getPanelSlidesTimeline(page)).length, { timeout: 10_000 })
      .toBe(2);
    const summaryEntries = await getPanelSlideSummaryEntries(page);
    const titleEntries = await getPanelSlideTitleEntries(page);
    await expect
      .poll(
        async () => (await getPanelSlideDescriptions(page)).map(([, text]) => text).join("\n"),
        {
      ...
      .toContain("portable classified device");
    expect(summaryEntries[0]?.[1] ?? "").toContain("portable classified device");
    expect(titleEntries[0]?.[1] ?? "").toBe("Microwave briefing");
    const descriptions = await getPanelSlideDescriptions(page);
    expect(descriptions[0]?.[1] ?? "").toContain("portable classified device");
    expect(descriptions[1]?.[1] ?? "").toContain("no longer looks like coincidence");
    expect(descriptions.some(([, text]) => text.includes("raw transcript fallback"))).toBe(false);
    await expect(
      page.locator(
        'img.slideStrip__thumbImage[data-loaded="true"], img.slideInline__thumbImage[data-loaded="true"]',
      ),
